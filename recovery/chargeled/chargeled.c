 ...
 * Adapted for Samsung leds by CastagnaIT @ Gottardo Stefano
 *
...
#define LED_PATTERN "/sys/class/sec/led/led_pattern"
#define LED_LOWPOWERMODE "/sys/class/sec/led/led_lowpower"

#define LED_PATTERN_OFF "0"
#define LED_PATTERN_CHARGING "1"
#define LED_PATTERN_FULLY_CHARGED "5"

#define LED_LOWPOWER_OFF "0"
#define LED_LOWPOWER_ON "1"
...
static void update_led(int charge_status) {
    FILE *led_lp, *led_pttrn;
    led_lp = fopen(LED_LOWPOWERMODE, "w");
    if (!led_lp) {
        KLOG_ERROR(LOG_TAG, "%s: could not open led_lowpower: %s\n",
            __func__, LED_LOWPOWERMODE);
        return;
    } else {
        led_pttrn = fopen(LED_PATTERN, "w");
        if (!led_pttrn) {
            fclose(led_lp);
            KLOG_ERROR(LOG_TAG, "%s: could not open led_pattern: %s\n",
                __func__, LED_PATTERN);
            return;
        }
    }
    ...
    switch (charge_status) {
        case BATTERY_STATUS_CHARGING:
            fputs(LED_LOWPOWER_OFF, led_lp);
            fputs(LED_PATTERN_CHARGING, led_pttrn);
            break;
        case BATTERY_STATUS_FULL:
            fputs(LED_LOWPOWER_OFF, led_lp);
            fputs(LED_PATTERN_FULLY_CHARGED, led_pttrn);
            break;
        default:
            fputs(LED_LOWPOWER_ON, led_lp);
            fputs(LED_PATTERN_OFF, led_pttrn);
            break;
    }
